...
# ---------------------------------------------------------------------------
# Minimum NVIDIA driver and CUDA versions required by vLLM + Qwen3.5-9B AWQ
#
...
check_gpu_hardware() {
  ...
  Qwen3.5-9B AWQ 4-bit quantisation. There is no CPU fallback.
"
    fi
...
"
  ...
  vLLM requires CUDA >= ${min_cuda} to run Qwen3.5-9B AWQ 4-bit.
