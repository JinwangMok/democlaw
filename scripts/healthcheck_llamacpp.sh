#!/usr/bin/env bash
# =============================================================================
# healthcheck_llamacpp.sh — Smoke-test/poll until the llama.cpp model is fully loaded
#
# Repeatedly polls the llama.cpp /v1/models endpoint until the expected model
...
#   /health responds HTTP 200 as soon as the llama.cpp HTTP server starts, but
...
#
#   LLAMACPP_HOST_PORT              Port the llama.cpp server is published on the host.
#                               Default: 8000
#
#   LLAMACPP_BASE_URL               Full base URL override (e.g. http://host:8001).
#                               Default: http://localhost:<LLAMACPP_HOST_PORT>
#
...
#
#   LLAMACPP_HEALTH_TIMEOUT         Maximum total seconds to wait before giving up.
#                               Default: 600  (model loading can take minutes)
#
#   LLAMACPP_HEALTH_INTERVAL        Seconds between polling attempts.
#                               Default: 5
#
#   LLAMACPP_HEALTH_CURL_TIMEOUT    Per-request curl timeout in seconds.
#                               Default: 10
#
# Exit codes:
#   0 — /v1/models confirmed the expected model is loaded and ready
#   1 — Timeout exceeded; model did not become ready within LLAMACPP_HEALTH_TIMEOUT
#   2 — Prerequisite missing (curl not found in PATH)
#
# Usage:
#   ./scripts/healthcheck_llamacpp.sh
#   LLAMACPP_HOST_PORT=8001 ./scripts/healthcheck_llamacpp.sh
#   MODEL_NAME=Qwen/Qwen3-4B-AWQ LLAMACPP_HEALTH_TIMEOUT=900 ./scripts/healthcheck_llamacpp.sh
#   MODEL_NAME="" ./scripts/healthcheck_llamacpp.sh   # accept any loaded model
# =============================================================================
set -euo pipefail
...
LLAMACPP_HEALTH_TIMEOUT="${LLAMACPP_HEALTH_TIMEOUT:-600}"
LLAMACPP_HEALTH_INTERVAL="${LLAMACPP_HEALTH_INTERVAL:-5}"
LLAMACPP_HEALTH_CURL_TIMEOUT="${LLAMACPP_HEALTH_CURL_TIMEOUT:-10}"

MODELS_URL="${LLAMACPP_BASE_URL}/v1/models"
HEALTH_URL="${LLAMACPP_BASE_URL}/health"

# ---------------------------------------------------------------------------
# Output helpers
# ---------------------------------------------------------------------------
log()   { echo "[healthcheck-llamacpp] $*"; }
warn()  { echo "[healthcheck-llamacpp] WARNING: $*" >&2; }
error() { echo "[healthcheck-llamacpp] ERROR: $*" >&2; }
...
query_models() {
    ...
    http_code=$(curl -sf -o "${tmpfile}" -w "%{http_code}" \
        --max-time "${LLAMACPP_HEALTH_CURL_TIMEOUT}" \
...
# Main
# ---------------------------------------------------------------------------
log "Polling llama.cpp /v1/models for model readiness"
log "  URL              : ${MODELS_URL}"
log "  Expected model   : ${MODEL_NAME:-<any>}"
log "  Timeout          : ${LLAMACPP_HEALTH_TIMEOUT}s"
log "  Interval         : ${LLAMACPP_HEALTH_INTERVAL}s"
log "  Per-request curl : ${LLAMACPP_HEALTH_CURL_TIMEOUT}s"
log ""
...
while [ "${elapsed}" -lt "${LLAMACPP_HEALTH_TIMEOUT}" ]; do
    ...
    remaining=$(( LLAMACPP_HEALTH_TIMEOUT - elapsed ))
    log "  ... not ready (${elapsed}s elapsed, ${remaining}s remaining) — ${_LAST_STATUS}"

    sleep "${LLAMACPP_HEALTH_INTERVAL}"
    elapsed=$(( elapsed + LLAMACPP_HEALTH_INTERVAL ))
done
...
# ---------------------------------------------------------------------------
# Timeout reached — model did not become ready within LLAMACPP_HEALTH_TIMEOUT
# ---------------------------------------------------------------------------
warn ""
warn "TIMEOUT: llama.cpp model '${MODEL_NAME:-<any>}' did not become ready within ${LLAMACPP_HEALTH_TIMEOUT}s."
warn "  /v1/models URL  : ${MODELS_URL}"
warn "  /health URL     : ${HEALTH_URL}"
warn "  Last status     : ${_LAST_STATUS}"
warn ""
warn "Possible causes:"
warn "  1. The llama.cpp container is not running. Start it with:"
...
warn "       LLAMACPP_HEALTH_TIMEOUT=900 ./scripts/healthcheck_llamacpp.sh"
...
warn "       docker logs democlaw-llamacpp --tail 50"
warn "       podman logs democlaw-llamacpp --tail 50"
warn "  5. Port ${LLAMACPP_HOST_PORT} is blocked or bound by another process."
warn "       ss -tlnp | grep ${LLAMACPP_HOST_PORT}"
warn ""
exit 1
